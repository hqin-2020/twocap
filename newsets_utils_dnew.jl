...
function dstar_twocapitals!(d1::Array{Float64,2},
    ...
    for i=1:IJ
        ...
        function f(d1x)
            d2_temp = delta*(1-p)/(1-phi1*d1x)/(1-p-Vr[i]) - (1-p)*(A1-d1x);
            d2x = A2 - d2_temp/p
            return delta*p/((A1-d1x)*(1-p) + (A2-d2x)*p) - (p+Vr[i])*(1-phi2*d2x)
        end

        # d1_root = find_zero(f, (0.001, A1));
        x0 = 0.03;
        d1_root = find_zero(f, x0, Roots.Order1());
        d2y = delta*(1-p)/(1-phi1*d1_root)/(1-p-Vr[i]) - (1-p)*(A1-d1_root);
        d2_root = A2 - d2y/p;
        d1[i] = d1_root;
        d2[i] = d2_root;
...
function value_function_twocapitals(ell::Float64,
      ...
      for n=1:maxit
          ...
          dstar_twocapitals!(d1_F, d2_F, Vr_F, pii, IJ, V, model);
          dstar_twocapitals!(d1_B, d2_B, Vr_B, pii, IJ, V, model);
